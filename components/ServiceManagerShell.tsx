...
const defaultStats = [
  { label: 'Map coverage', value: '92%', trend: '+4% expanded' },
  { label: 'Service SLA', value: '96%', trend: '+2% maintained' },
  { label: 'Active jobs', value: '48', trend: '+8 dispatched' },
  { label: 'Technicians', value: '26', trend: '+1 on duty' },
]
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={2} sx={{ py: 2 }}>
            <Stack direction="row" alignItems="center" spacing={1.5}>
              ...
              <Box>
                ...
                <Typography variant="body2" color="#8b949e">Operations control center</Typography>
              </Box>
            </Stack>

            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} sx={{ width: { xs: '100%', md: 'auto' }, alignItems: { xs: 'flex-start', sm: 'center' } }}>
              <TextField size="small" placeholder="Search dispatches" sx={{ minWidth: { xs: '100%', sm: 220 }, '& .MuiOutlinedInput-root': { bgcolor: '#0d1117', color: '#f0f6fc', borderColor: '#30363d' } }} />
              <Stack direction="row" spacing={1}>
                <Button variant="outlined" size="small" sx={{ color: '#f0f6fc', borderColor: '#30363d' }}>🔔 4</Button>
                <Button variant="outlined" size="small" sx={{ color: '#f0f6fc', borderColor: '#30363d' }}>💬 6</Button>
              ...
              <Stack direction="row" alignItems="center" spacing={1.25}>
                ...
                <Box>
                  <Typography variant="caption" color="#f0f6fc" fontWeight={700}>{session?.name || 'Manager Profile'}</Typography>
                  <Typography variant="caption" color="#8b949e" display="block">{session ? `${session.name} • Manager` : 'Guest'}</Typography>
                </Box>
          ...
          <Grid item xs={12} lg={3}>
            <Paper sx={{ p: 2.5, borderRadius: 4, height: '100%', transition: 'all 0.2s ease', bgcolor: '#161b22', border: '1px solid #30363d', boxShadow: 'none' }}>
              <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1.5 }}>
                <Box>
                  <Typography variant="overline" color="#58a6ff" fontWeight={700}>Control center</Typography>
                  <Typography variant="h5" fontWeight={800} color="#f0f6fc">{isSidebarCollapsed ? 'Menu' : 'Manager'}</Typography>
                </Box>
            ...
            <Stack spacing={3}>
              <Paper sx={{ p: { xs: 2.5, md: 3 }, borderRadius: 4, bgcolor: '#161b22', border: '1px solid #30363d', boxShadow: 'none' }}>
                <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={2}>
                  <Box>
                    <Typography variant="overline" color="#58a6ff" fontWeight={700}>Operations view</Typography>
                  ...
                  <Chip label="Manager" size="small" sx={{ bgcolor: '#1f6feb', color: '#fff' }} />
                </Stack>
              </Paper>
              ...
              <Grid container spacing={2}>
                {stats.map((stat, index) => (
                  <Grid item xs={12} sm={6} lg={3} key={stat.label}>
                    <Box component={Link} href="/service-manager-dashboard" sx={{ textDecoration: 'none' }}>
              ...
              <Card sx={{ borderRadius: 4, bgcolor: '#161b22', border: '1px solid #30363d', boxShadow: 'none' }}>
                <CardContent>
                  <Typography variant="h6" fontWeight={700} color="#f0f6fc" sx={{ mb: 2 }}>Team performance</Typography>
                  <Box sx={{ mt: 1 }}>
