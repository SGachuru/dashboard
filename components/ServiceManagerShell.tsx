...
interface ServiceManagerShellProps {
  title: string
  subtitle: string
  active: string
  children?: ReactNode
...
const defaultStats: MetricItem[] = [
  { label: 'Active jobs', value: '128', trend: '+9%', trendUp: true, icon: '🗺️' },
  { label: 'Technicians on route', value: '24', trend: '3 need reassignment', trendUp: false, icon: '👷' },
  { label: 'On-time completion', value: '96%', trend: '+2%', trendUp: true, icon: '✅' },
  { label: 'Escalations', value: '7', trend: '2 urgent', trendUp: false, icon: '⚠️' },
]
        ...
        <Box sx={{ p: 2.5, display: 'flex', alignItems: 'center', justifyContent: isSidebarOpen ? 'space-between' : 'center', minHeight: 72 }}>
          {isSidebarOpen ? (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Box sx={{ width: 36, height: 36, borderRadius: 2, bgcolor: '#1976d2', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: '0.8rem', flexShrink: 0 }}>
                PP
              </Box>
            ...
            <Box sx={{ width: 36, height: 36, borderRadius: 2, bgcolor: '#1976d2', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: '0.8rem' }}>
              PP
            </Box>
          )}
        </Box>
            ...
            <Card sx={{ bgcolor: '#1e293b', border: '1px solid #334155', borderRadius: 2 }}>
              <CardContent sx={{ p: 2 }}>
                ...
                <Typography variant="subtitle2" fontWeight={700} color="#f8fafc" sx={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {session?.name || 'Manager Profile'}
                </Typography>
                <Typography variant="caption" color="#64748b" sx={{ display: 'block' }}>
                  {session ? `${session.name} • Manager` : 'Guest'}
                </Typography>
                <Button size="small" variant="outlined" sx={{ mt: 1.5, width: '100%', borderColor: '#334155', color: '#94a3b8', '&:hover': { borderColor: '#1976d2', color: '#f8fafc' } }} onClick={handleLogout}>
                  Sign out
                </Button>
              </CardContent>
            </Card>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flex: 1 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, bgcolor: '#1e293b', borderRadius: 2, px: 2, py: 0.75, maxWidth: 420, flex: 1, border: '1px solid #334155' }}>
              ...
              <InputBase placeholder="Search dispatches, technicians..." sx={{ color: '#f8fafc', width: '100%', '& .MuiInputBase-input::placeholder': { color: '#64748b', opacity: 1 } }} />
            </Box>
          </Box>
          ...
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            <Avatar sx={{ width: 36, height: 36, bgcolor: '#1976d2', fontSize: '0.9rem', fontWeight: 700 }}>
              {session?.name?.[0] || 'U'}
            </Avatar>
          </Stack>
        </Paper>
        ...
        <Container maxWidth="xl" sx={{ py: 4, flex: 1 }}>
          <Stack spacing={4}>
            <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={2}>
              <Box>
                <Chip label="Operations" size="small" sx={{ bgcolor: '#1976d218', color: '#1976d2', fontWeight: 700, mb: 1.5 }} />
              ...
              <Button variant="contained" sx={{ bgcolor: '#1976d2', color: '#fff', fontWeight: 700, px: 3, '&:hover': { bgcolor: '#1565c0' } }}>
                + New Dispatch
              </Button>
            </Stack>
            ...
            <Grid container spacing={2.5}>
              {(stats || defaultStats).map((stat) => {
                const isWarning = stat.label === 'Escalations'
                const sc = isWarning ? { bg: '#ef444418', color: '#ef4444' } : stat.trendUp ? { bg: '#22c55e18', color: '#22c55e' } : { bg: '#f59e0b18', color: '#f59e0b' }
                return (
                  <Grid item xs={12} sm={6} md={3} key={stat.label}>
                    <Card sx={{ borderRadius: 3, bgcolor: '#1e293b', border: '1px solid #334155', boxShadow: 'none', transition: 'all 0.2s ease', '&:hover': { borderColor: '#475569', transform: 'translateY(-2px)' } }}>
                      <CardContent>
                        ...
                        <Typography variant="caption" color={sc.color} sx={{ fontWeight: 600 }}>
                          {stat.trend}
                        </Typography>
                      </CardContent>
                    </Card>
