...
const defaultStats = [
  { label: 'Ad inventory', value: '24', trend: '+3 placements' },
  { label: 'Lead generation', value: '312', trend: '+18% conversions' },
  { label: 'Campaigns', value: '8', trend: '+2 active' },
  { label: 'Partner score', value: '4.8/5', trend: '+0.2 rating' },
]
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={2} sx={{ py: 2 }}>
            <Stack direction="row" alignItems="center" spacing={1.5}>
              ...
              <Box>
                ...
                <Typography variant="body2" color="#8b949e">Partner workspace</Typography>
              </Box>
            </Stack>

            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} sx={{ width: { xs: '100%', md: 'auto' }, alignItems: { xs: 'flex-start', sm: 'center' } }}>
              <TextField size="small" placeholder="Search campaigns" sx={{ minWidth: { xs: '100%', sm: 220 }, '& .MuiOutlinedInput-root': { bgcolor: '#0d1117', color: '#f0f6fc', borderColor: '#30363d' } }} />
              ...
              <Stack direction="row" alignItems="center" spacing={1.25}>
                ...
                <Box>
                  <Typography variant="caption" color="#f0f6fc" fontWeight={700}>{session?.name || 'Partner Profile'}</Typography>
                  <Typography variant="caption" color="#8b949e" display="block">{session ? `${session.name} • Partner` : 'Guest'}</Typography>
                </Box>
          ...
          <Grid item xs={12} lg={3}>
            <Paper sx={{ p: 2.5, borderRadius: 4, height: '100%', transition: 'all 0.2s ease', bgcolor: '#161b22', border: '1px solid #30363d', boxShadow: 'none' }}>
              <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1.5 }}>
                <Box>
                  <Typography variant="overline" color="#58a6ff" fontWeight={700}>Partner workspace</Typography>
                  <Typography variant="h5" fontWeight={800} color="#f0f6fc">{isSidebarCollapsed ? 'Menu' : 'Partner'}</Typography>
                </Box>
            ...
            <Stack spacing={3}>
              <Paper sx={{ p: { xs: 2.5, md: 3 }, borderRadius: 4, bgcolor: '#161b22', border: '1px solid #30363d', boxShadow: 'none' }}>
                <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={2}>
                  <Box>
                    <Typography variant="overline" color="#58a6ff" fontWeight={700}>Partner view</Typography>
                  ...
                  <Chip label="Partner" size="small" sx={{ bgcolor: '#1f6feb', color: '#fff' }} />
                </Stack>
              </Paper>
